...
const sidebars: SidebarsConfig = {
  docsSidebar: [
    'intro',
    'overview',
    { type: 'autogenerated', dirName: 'shapes' },
    { type: 'autogenerated', dirName: 'styling' },
    { type: 'autogenerated', dirName: 'events' },
    { type: 'autogenerated', dirName: 'drag-and-drop' },
    { type: 'autogenerated', dirName: 'clipping' },
    { type: 'autogenerated', dirName: 'groups-and-layers' },
    { type: 'autogenerated', dirName: 'filters' },
    { type: 'autogenerated', dirName: 'tweens' },
    { type: 'autogenerated', dirName: 'animations' },
    { type: 'autogenerated', dirName: 'selectors' },
    { type: 'autogenerated', dirName: 'data-and-serialization' },
...
Expected: `依赖与安全` PASS；`构建健全性` 仍 FAIL，报告 `文档页只有 N 个，期望至少 97 个`（此时尚无内容，N 接近 1）
...
/** 章节标题沿用原 dumi frontmatter 的 group.title，顺序沿用 group.order。 */
const CATEGORY_LABELS = {
  shapes: { label: '图形', position: 1 },
  styling: { label: '样式', position: 2 },
  events: { label: '事件', position: 3 },
  'drag-and-drop': { label: '拖拽/释放', position: 4 },
  clipping: { label: '剪辑', position: 5 },
  'groups-and-layers': { label: '分组、图层', position: 6 },
  filters: { label: '滤镜', position: 7 },
  tweens: { label: '补间动画', position: 8 },
  animations: { label: '动画', position: 9 },
  selectors: { label: '选择器', position: 10 },
  'data-and-serialization': { label: '数据序列化', position: 11 },
  performance: { label: '性能优化', position: 12 },
}
...
# 96 = 97 减去首页 docs/index.md，首页已由 src/pages/index.tsx 承接
...
（其余 94 条照规格 §7.2 表格续写。`/` → `/` 一条为恒等映射，**不写入**，写入会造成重定向循环。）
...
```bash
npm install @docusaurus/plugin-google-gtag@3.10.2
```
...
```bash
git add src/config/analytics.ts docusaurus.config.ts package.json package-lock.json test/checks/analytics.js test/verify.js
