...
- **C3 实为 23 页而非 21。** 表内各项相加本就是 22；且 `filters` 实缺 **14** 页而非 13——
  官方的 `RGB` 与本站的 `rgba` 是两个不同的滤镜，算法完全无关
  （`RGB` 按亮度把整图重染成单色调，`RGBA` 是 `目标色×alpha + 原色×(1-alpha)` 的线性混合），
  只是共用 `red`/`green`/`blue` 三个访问器，不能相互抵充。
- **C3 验收改为「12 个章节页数不少于官方，差额为本站原创页」。** 本站有 3 页官方没有的原创页
  （`filters/css-filters`、`filters/rgba`、`performance/shape-redraw`），
  硬要求「与官方一致」会逼着删掉它们。
- **全站缺口实为 170 页**（277 官方 − 107 已对齐），拆分如下：

| 批次 | 内容 | 页数 | 状态 |
|---|---|---:|---|
| **P6** | 12 个既有章节 +23、`guides` 2、`nodejs` 1、根级 5 | **31** | **已完成** |
| P7 | `react` 19、`vue` 13、`svelte` 16、`angular` 12 | 60 | 待做 |
| P8 | `sandbox` 69 + 根级 `sandbox` 画廊索引 1 | 70 | 待做，其中 26 个演示 HTML 已在仓库 |
| P9 | `posts` 9 | 9 | 待做 |

全部完成后本站 277 + 3 = **280 页**。P6 结束时为 **141 页**。
...
**P5 已完成全站覆盖**：110 页全部改造为三段结构，`ENFORCED_PREFIXES` 收敛为
单个 `'docs/'`，此后新增页面自动纳入校验，不再需要维护列表。
改造后全站散文中位数 929 字（改造前 137 字），
够正文内广告位门槛的页面 110/110（改造前 2/97）。
